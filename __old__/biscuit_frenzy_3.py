...
def run_script():
    ...
    # Construct the path to the Jira script
    script_path = os.path.join(current_directory, 'jira_api_test_30A.py')

    # Constructing the command
    cmd = [sys.executable, script_path, url, username, token, project_key, str(start_range), str(end_range)]

    # Open a new PowerShell window and run the script with the provided arguments
    subprocess.Popen(["powershell", "-Command", "Start-Process", "powershell", "-ArgumentList", '-NoExit', '-Command', ' '.join(cmd)])
